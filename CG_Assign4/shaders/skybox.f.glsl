...
void main() {
    if (sun_position.y < 0.0) {
        fColor = texture2D(night_texture, texCoord);

    } else if (sun_position.y < 100.0) {
        vec4 night_color = texture2D(night_texture, texCoord);
        vec4 sunset_color = texture2D(sunset_texture, texCoord);
        fColor = mix(night_color, sunset_color, sun_position.y / 100.0);

    } else if (sun_position.y < 200.0) {
        vec4 sunset_color = texture2D(sunset_texture, texCoord);
        vec4 day_color = texture2D(day_texture, texCoord);
